---
title: IrisCTF: PCAP Forensics Layer 2 Traffic
tag: CTF
date: 2023-09-01
readTime: 5 min
excerpt: A walkthrough of cracking WPA/PSK authentication from a layer 2 pcap capture using aircrack-ng and decrypting 802.11 traffic in Wireshark to retrieve a CTF flag
mitre: N/A
---
...
![Alt text](/images/crack.png)

The wifi password as cracked by aircrack-ng is **billybob1**.

Now that i got the wifi password, the next step is to use it to unencrypt the data packets in the capture. On wireshark, navigate through **Edit -> Preferences -> Protocols -> IEEE 802.11**.

On this view, select **Enable Decryption**, go to **decryption keys -> Edit**. To add a new decryption key, select **New** and in the **key type**, select WPA-PWD on the options listed which is as according to the Router's configuration. In the **Key** tab provide the appropriate password and the ssid of the network with a colon to separate the two values.
...
- [EAPOL - Extensible Authentication Protocol over LAN](https://ipcisco.com/lesson/eapol-extensible-authentication-protocol-over-lan)